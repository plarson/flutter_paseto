...
extension on SecretKey {
  ...
  Future<SymmetricSubkeysV3> deriveSubkeysV3({
    required Mac nonce,
  }) async {
    final secretKey = await extractBytes();
    final hkdf = Hkdf(
      hmac: Hmac.sha384(),
      outputLength: LocalV3.nonceLength,
    );
...
@immutable
class SymmetricSubkeysV3 {
  const SymmetricSubkeysV3({
